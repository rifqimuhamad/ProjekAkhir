<div class="wrapper row-padding">
	...
	<form action="<?php echo BASE_URL."module/user/action.php?user_id=$user_id"?>" method="POST">
		...
		<div class="element-form">
			<label>Level</label>	
			<span>
				<input type="radio" value="Pegawai" name="level" <?php if($level == "Pegawai"){ echo "checked"; } ?> /> Superadmin
				<input type="radio" value="Pembeli" name="level" <?php if($level == "Pembeli"){ echo "checked"; } ?> /> Customer			
			</span>
		</div>	
